...
After entering the email and password, you will be asked to enter a **One-Time Password (OTP)** generated by the Authenticator App. MFA is enabled using the **Authenticator App** as the primary factor, because Auth0 **does not allow email** to be used as the only MFA method.

 To use **email verification** instead, click **“Try another method”** and select the Email option. 
